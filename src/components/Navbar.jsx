    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-dark/80 backdrop-blur-xl border-b border-dark-border/40 shadow-lg shadow-black/20">
      <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          {navLinks.map(l => (
            <a key={l.href} href={l.href} className="text-sm text-gray-400 hover:text-accent-pink transition-colors">
              {l.label}
            </a>
          ))}
          ...
          {socialLinks.map(({ href, icon: Icon, label }) => (
            <a key={label} href={href} target="_blank" rel="noopener noreferrer"
              className="text-gray-500 hover:text-accent-green transition-colors" aria-label={label}>
          ...
          <a href="#" className="flex items-center gap-1.5 px-3 py-1.5 bg-accent-green/10 border border-accent-green/20 rounded-lg text-xs text-accent-green hover:bg-accent-green/20 transition-all font-medium">
          ...
          <div className="px-4 py-4 space-y-3">
            {navLinks.map(l => (
              <a key={l.href} href={l.href} onClick={() => setOpen(false)}
                className="block text-gray-300 hover:text-accent-pink transition-colors">
                {l.label}
              </a>
            ))}
            <div className="flex gap-3 pt-2 border-t border-dark-border/30">
              {socialLinks.map(({ href, icon: Icon, label }) => (
                <a key={label} href={href} target="_blank" rel="noopener noreferrer"
                  className="p-2 text-gray-400 hover:text-accent-green transition-colors" aria-label={label}>
